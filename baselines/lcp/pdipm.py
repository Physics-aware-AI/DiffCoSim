...
from .util import efficient_btriunpack, get_sizes, bdiag
...
def solve_kkt(Q_LU, d, G, A, S_LU, rx, rs, rz, ry):
    ...
    invQ_rx = (rx.unsqueeze(2).lu_solve(*Q_LU)).squeeze(2)  # Q-1 rx
    if neq > 0:
        # A Q-1 rx - ry
        # G Q-1 rx + rs / d - rz
    ...
    w = -(h.unsqueeze(2).lu_solve(*S_LU)).squeeze(2)  # S-1 h =
    ...
    dx = (g1.unsqueeze(2).lu_solve(*Q_LU)).squeeze(2)  # Q-1 g1 = - Q-1 AT S-1 h
    ds = g2 / d  # g2 / d = (-rs - w) / d
    dz = w[:, neq:]
