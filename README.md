...
All the data can be generated by [datasets.py](./datasets/datasets.py). It might take a while to generate the dataset, so we also host the dataset on github releases. You can download the dataset [here](). Please unzip the dataset into the `./datasets` folder.
...
Pre-trained models can be downloaded from github releases ([Here's the link]()). 
...
The following animation are prediction animations by our CM-CD-CLNN model, which is generated in [this](./analysis/plot_prediction_gifs.ipynb) notebook.
