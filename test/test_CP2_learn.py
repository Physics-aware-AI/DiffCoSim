...
def test_CP2_learn_0():
    checkpoint_path = os.path.join(
        PARENT_DIR,
        "logs",
        "CP2_ground_homo_cor0.5_mu0.5_N800",
        "version_0",
        "epoch=276.ckpt"
    ) 
    ...
    ani.save(os.path.join(THIS_DIR, 'CP2_learn_0_true_zts.gif'))

    ani = model.body.animate(log["pred_zts"], 0)
    ani.save(os.path.join(THIS_DIR, "CP2_learn_0_pred_zts.gif"))

    assert 1

def test_CP2_learn_1():
    checkpoint_path = os.path.join(
        PARENT_DIR,
        "logs",
        "CP2_ground_homo_cor1_mu0_N800",
        "version_0",
        "epoch=713.ckpt"
    ) 
    model = Model.load_from_checkpoint(checkpoint_path)
    ...
    ani = model.body.animate(log["true_zts"], 0)
    ani.save(os.path.join(THIS_DIR, 'CP2_learn_1_true_zts.gif'))

    ani = model.body.animate(log["pred_zts"], 0)
    ani.save(os.path.join(THIS_DIR, "CP2_learn_1_pred_zts.gif"))

    assert 1

def test_CP2_learn_2():
    checkpoint_path = os.path.join(
        PARENT_DIR,
        "logs",
        "CP2_ground_homo_cor1_mu1_N800",
        "version_0",
        "epoch=914.ckpt"
    ) 
    model = Model.load_from_checkpoint(checkpoint_path)
    ...
    ani = model.body.animate(log["true_zts"], 0)
    ani.save(os.path.join(THIS_DIR, 'CP2_learn_2_true_zts.gif'))

    ani = model.body.animate(log["pred_zts"], 0)
    ani.save(os.path.join(THIS_DIR, "CP2_learn_2_pred_zts.gif"))
