...
def test_Gyro_learn_0():
    checkpoint_path = os.path.join(
        PARENT_DIR,
        "logs",
        "Gyro_homo_cor1_mu0_N800",
        "version_1",
        "epoch=856.ckpt"
    ) 
    model = Model.load_from_checkpoint(checkpoint_path)
    print(torch.exp(model.model.m_params["3"]))
    ...
    ani.save(os.path.join(THIS_DIR, 'Gyro_learn_0_true_zts.gif'))

    ani = model.body.animate(log["pred_zts"], 0)
    ani.save(os.path.join(THIS_DIR, "Gyro_learn_0_pred_zts.gif"))
