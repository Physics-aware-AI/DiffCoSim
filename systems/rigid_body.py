...
class RigidBody(object):
    ...
    def add_impulse(self, xv):
        ...
        for bs_idx in torch.nonzero(is_cld).squeeze(1):
            ...
            #     assert torch.allclose(e, (e>0.5).to(dtype=e.dtype))
            ...
            # else:
            #     Minv_sqrt = torch.cholesky(self.Minv) # (n, n)
            ...
            # else:
            #     Minv_Jac_T = (self.Minv @ Jac.reshape(n_cld*d, n*d).t().reshape(n, d*n_cld*d)).reshape(n*d, n_cld*d)
    ...
    def get_contact_Jacobian(self, bs_idx, x, cld_ij_ids, cld_bdry_ids):
        ...
        p1_bdry = x[bs_idx, cld_bdry_ids[:, 0]] # n_cld_bdry, d
        ...
        pi, pj = x[bs_idx, cld_ij_ids].unbind(dim=1) # n_cld_ij, 2, d -> n_cld_ij, d

        p1 = torch.cat([pi, p1_bdry], dim=0) # n_cld, d
        p2 = torch.cat([pj, p2_bdry], dim=0) 

        # local frame in contact space, obj2 for now
        e_n = (p2 - p1) / ((p2 - p1) ** 2).sum(1, keepdim=True).sqrt() # n_cld, 2
        e_t = torch.zeros_like(e_n) # n_cld, 2
        e_t[..., 0], e_t[..., 1] = -e_n[..., 1], e_n[..., 0]

        # friction or no friction?
        # Todo: check edge cases
        Jac = torch.zeros([n_cld, d, n, d], device=x.device, dtype=x.dtype)
        ...
        for cid, (i, _) in enumerate(cld_bdry_ids):
            Jac[n_cld_ij+cid, 0, i, :] = - e_n[cid]
            Jac[n_cld_ij+cid, 1, i, :] = - e_t[cid]

        return Jac

    def update_v_w_J_e(self, bs_idx, v, Jac, Jac_v, v_star, mu, cor, DPhi):
        n_cld, d, n, _ = Jac.shape
        C = DPhi.shape[-1]
